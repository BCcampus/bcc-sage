...
<section class="d-flex flex-row flex-wrap">
	@foreach(\App\App::getLatestNews( $args ) as $feature)
		...
		<article class="row-fluid border-left border-bottom border-right" itemscope itemtype="http://schema.org/Article">
			<p class="text-uppercase pt-3 px-3 font-size-sm"><time itemprop="datePublished" datetime="{{ get_post_time('c', true, $feature->ID) }}">{{ get_the_date('',$feature->ID) }}</time> &nbsp;<i class="fa fa-circle green small"></i>&nbsp; <a href="<?php echo esc_url( get_category_link( $cat[0]->term_id ) ) ;?>">{{$cat[0]->name}}</a></p>
			<p class="px-3"><?php echo wp_trim_words( $feature->post_content, '30', "<a href='{$link}'>&hellip;<i class='fa fa-arrow-right'></i></a>" );?></p>
		</article>
	@endforeach
</section>
