...
<section class="container-fluid">
	@foreach(\App\App::getLatestNews( $args ) as $feature)
		...
		<article class="row border-left border-bottom border-right" itemscope itemtype="http://schema.org/Article">
			<small><p class="text-uppercase pt-3 pad-left pad-right"><time itemprop="datePublished" class="updated" datetime="{{ get_post_time('c', true, $feature->ID) }}">{{ get_the_date('',$feature->ID) }}</time> &nbsp;<i class="fa fa-circle green small"></i>&nbsp; <a href="<?php echo esc_url( get_category_link( $cat[0]->term_id ) ) ;?>">{{$cat[0]->name}}</a></p></small>
